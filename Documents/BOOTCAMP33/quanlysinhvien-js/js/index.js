...
function createStudent() {
  ...
  */
  ...
  valid &=
    kiemTraRong(student.id, "#spanMaSV", "Mã SV") &
    kiemTraRong(student.name, "#spanTenSV", "Tên sinh viên") &
    kiemTraRong(student.email, "#spanEmailSV", "email") &
    kiemTraRong(student.dob, "#spanNgaySinh", "Ngày sinh") &
    kiemTraRong(student.math, "#spanToan", "Điểm toán") &
    kiemTraRong(student.physic, "#spanLy", "Điểm lý") &
    kiemTraRong(student.chemistry, "#spanHoa", "Điểm hoá");

  // Kiểm tra ký tự
  if (kiemTraRong(student.name, "#spanTenSV", "Tên sinh viên")) {
    valid &= kiemTraKyTu(student.name, "#spanTenSV", " Tên sinh viên");
  }
  valid &=
    kiemTraSo(student.math, "#spanToan", " Điểm toán") &
    kiemTraSo(student.physic, "#spanLy", " Điểm Lý") &
    kiemTraSo(student.chemistry, "#spanHoa", " Điểm Hóa");
  valid &= kiemTraEmail(student.email, "#spanEmailSV", "Email");

  //kiểm tra độ dài
  //từ 2 --> 5 kí tự
  valid &= kiemTraDoDai(student.id, "#spanMaSV", "Mã SV", 2, 5);

  //kiểm tra giá trị
  valid &=
    kiemTraGiaTri(student.math, "#spanToan", " Điểm Toán ", 0, 10) &
    kiemTraGiaTri(student.physic, "#spanLy", " Điểm Lý ", 0, 10) &
    kiemTraGiaTri(student.chemistry, "#spanHoa", " Điểm Hóa ", 0, 10);
  if (!valid) {
    return;
  }

  // Nghiêng cứu thư viện Sweet Alert
  if (confirm("bạn có muốn thêm sinh viên")) {
 ...
 */
function renderStudentList(arrSV) {
  ...
  for (var index = 0; index < arrSV.length; index++) {
    ...
    var trSV = `
        <tr>
          ...
          <td>
            <button class="btn btn-danger" onclick="delStudent('${
              obSinhVien.id
            }')">Del</button>
